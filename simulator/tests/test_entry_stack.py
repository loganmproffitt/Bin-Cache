...
class TestEntryStack(unittest.TestCase):
    
    def setUp(self):
        """Runs before each test to provide a fresh BinStack instance."""
        self.stack = EntryStack(0, 0, depth=3)
    ...
    def test_stack_fifo_behavior(self):
        """Test that stack follows FIFO"""
        bin1 = Bin()
        bin2 = Bin()
        bin3 = Bin()
        ...
        self.assertEqual(self.stack.retrieve_bin(), bin1)  # Last added should be first out
        self.assertEqual(self.stack.retrieve_bin(), bin2)
        self.assertEqual(self.stack.retrieve_bin(), bin3)
